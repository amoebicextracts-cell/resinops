                ...
                <tbody>
                  ...
                  {filteredHbRows.map(r=>{const b=r.b;return(
                    <tr key={b.id}>
                      ...
                      <td>{(()=>{const totalG=(parseFloat(b.grades?.a?.weight)||0)+(parseFloat(b.grades?.aa?.weight)||0);const consumedG=gradeConsumedG(b.id,"a")+gradeConsumedG(b.id,"aa");return(<>{totalG>0?totalG+"g":"—"}{consumedG>0&&<HbTransferNote consumed={consumedG} remaining={Math.max(0,totalG-consumedG)} />}</>);})()}</td>
