          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <div style={{gridColumn:"span 2"}}>
              ...
              <select className="erp-inp" value={cocItemId} onChange={e=>setCocItemId(e.target.value)}>
                ...
                {items.map(it=><option key={it.id} value={it.id}>{it.n} ({it.cat})</option>)}
              </select>
            </div>
            ...
            <div style={{gridColumn:"span 2"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {["pass","fail","pending"].map(s=>(
                  <button key={s} onClick={()=>setCocForm(f=>({...f,status:s}))} style={{padding:"5px 16px",borderRadius:8,border:"none",cursor:"pointer",fontWeight:600,fontSize:12,background:cocForm.status===s?(s==="pass"?"var(--accent)":s==="fail"?"var(--danger)":"var(--amber)"):"var(--surface-2)",color:cocForm.status===s?"#fff":"var(--text-2)"}}>
